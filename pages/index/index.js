...
Page({
	...
	takePhoto() {
		...
		cameraContext.takePhoto({
			quality: 'high',
			success: res => {
				console.log(res, 'takePhoto', this.data.systemInfo);
				console.log(this.data.systemInfo.screenWidth, 'screenWidth');
				console.log(this.data.systemInfo.screenHeight, 'screenHeight');
				// .tempImagePath
	...
	drawImage: function (imagePath) {
		...
		my.getImageInfo({
			...
			success: info => {
				...
				ctx.fillText(new Date().toLocaleString().replace(/\//g,'-'), 20, height / 2 - 10)
				ctx.fillText(_this.data.address, 20, height / 2 + 20)
